...
#ETIQUETAS DE TEXTO(LABEL)
 titulo=Label(mainFrame, text="BIENVENIDO A SUPERMARK", font=("arial black",30), background="#F49739")
 titulo.grid(column=0, row=0, padx=10, pady=10 ,columnspan=2)

 nombrelabel=Label(mainFrame, text="Nombre: ",font=("impact",16), background="#F49739")
 nombrelabel.grid(column=0, row=1)
 passlabel=Label(mainFrame, text="Contraseña: ",font=("impact",16), background="#F49739")
...
#ENTRADAS DE TEXTO
 ...
 contraseñaEntry=Entry(mainFrame, textvariable=contraseña_usuario, show="*")
 contraseñaEntry.grid(column=1, row=2)

#BOTONES
 iniciarSesionButton=ttk.Style()
 iniciarSesionButton.configure("TButton",font=("arial black",10), foreground="#0068DD")
...
def iniciarSesion():
    for user in usuarios:
        if user.nombre==nombre_usuario.get():
           ...
           if test:
              Messagebox.showinfo("Conectado!", f"Se inicio sesión con [{user.nombre}] exito!!!")
           else:
...
def registrarUsuario():
    ...
    nuevo_usuario= usuarios(nom,contra) #en esta linea me marca error
    usuarios.append(nuevo_usuario)
    Messagebox.showinfo("Registro exitoso", f"Se registró el usuario [{nom}] !!!")
    nombre_usuario.set("")
    contraseña_usuario.set("") #error list object is not callable/ no me registra el usuario
